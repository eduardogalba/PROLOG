...
the task. Upon completion, it notifies the state owner telematically.

Additionally, the robot must adhere to the following rules:
...
### From well to a tree
`de_pozo_a_regar_arbol(A, D, NV, ND)`
...
`movimiento_desde_arbol(T, A, V, DA, DT)`
...
of tree identifiers indicating the order in which the robot has watered the trees in it.
